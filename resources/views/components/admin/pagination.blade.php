    ...
    <nav aria-label="Page navigation">
        <ul class="pagination">
            @if($paginationTools->previousPage !== 0)
                <li class="page-item">
                    <a class="page-link text-theme-2 waves-effect" href="{{ $paginationService->getPageUrl($paginationTools->url, $paginationTools->previousPage) }}" aria-label="Previous">
                ...
                @for($i = $paginationTools->currentPage -  $paginationTools->itemsBeforeAndAfter; $i < $paginationTools->currentPage; $i++)
                    @if($i > 0)
                        <li class="page-item">
                            <a class="page-link text-theme-2 waves-effect" href="{{ $paginationService->getPageUrl($paginationTools->url, $i) }}">
                                {{ $i }}
                            </a>
                        </li>
                    @endif
            ...
            <li class="page-item active">
                <span class="bg-theme-2 border-theme-2">
            ...
            @if($paginationTools->nextPage !== 0)
                <li class="page-item">
                    <a class="page-link text-theme-2 waves-effect" href="{{ $paginationService->getPageUrl($paginationTools->url, $paginationTools->nextPage) }}" aria-label="Next">
